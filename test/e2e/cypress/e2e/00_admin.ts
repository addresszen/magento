...
const navigateToSettings = () => {
  if (version == "2.2") {
    ...
    cy.get(".admin__page-nav-link.item-nav")
      .contains("Address Zen")
      .click();
    cy.wait(500);
    cy.get("#address_zen_required-head").click();
    cy.wait(500);
    return;
  }
  cy.visit("/index.php/admin/admin/system_config/edit/section/address_zen");
};
...
describe("Admin", () => {
  ...
  it("Can navigate to config page", () => {
    ...
    cy.url().should(
      "include",
      "/index.php/admin/admin/system_config/edit/section/address_zen"
    );
    cy.get("#address_zen_required_api_key")
      .clear({ force: true })
      .type(apiKey, { force: true });
    cy.get("#address_zen_admin-head").click();
    cy.get("#address_zen_admin_autocomplete_override")
      .clear({ force: true })
      .type('{{}"defaultCountry": "USA", "detectCountry": false}', {
        force: true,
      });
    cy.wait(500);
